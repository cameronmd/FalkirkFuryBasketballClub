...
- **Offline:** the app (including the SheetJS library and the bundled fixtures) is cached by a service worker after the first visit, so it opens without a connection. It uses a *network-first* strategy — when you're online it always loads the freshest files (so an update shows on the very next refresh), and only falls back to the cache when you're offline. Share links and calendar export are generated on-device and also work offline.
